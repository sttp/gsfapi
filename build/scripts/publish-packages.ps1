...
function Publish-Package([string]$package) {
    Invoke-Command -ScriptBlock {
        ...
        if ($env:NuGetCertFingerprint -ne $null) {
            ...
            & dotnet nuget sign $package --certificate-password $env:NuGetCertFingerprint --certificate-store-location LocalMachine --certificate-store-name AddressBook --timestamper http://timestamp.digicert.com
