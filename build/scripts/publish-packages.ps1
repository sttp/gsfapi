...
function Publish-Package([string]$package) {
    Invoke-Command -ScriptBlock {
        ...
        if ($env:NuGetCertFingerprint -ne $null) {
            ...
            & dotnet nuget sign $package -CertificateFingerprint $env:NuGetCertFingerprint -CertificateStoreLocation LocalMachine -CertificateStoreName AddressBook --timestamper http://timestamp.digicert.com
