...
//  04/25/2025 - J. Ritchie Carroll
//       Added support safe event handler invocation -- imported from Gemstone.
//
//******************************************************************************************************
...
public static class EventHandlerExtensions
{
    ...
    /// Any exceptions will be suppressed, see <see cref="Logger.SwallowException"/> and other overloads for custom exception handling.
    /// </remarks>
    ...
    /// <typeparam name="TEventArgs">Type derived from <see cref="EventArgs"/>.</typeparam>
    /// <param name="eventHandler">Source <see cref="EventHandler"/> to safely invoke.</param>
    /// <param name="exceptionHandler">Custom delegate to handle encountered exceptions; when set to <c>null</c>, exception will be suppressed, see <see cref="Logger.SwallowException"/>.</param>
    /// <param name="sender">Event source.</param>
    /// <param name="args">Event arguments.</param>
    /// <param name="parallel">Call event handlers in parallel, when attached handlers are greater than one.</param>
    /// <remarks>
    ...
    /// <typeparam name="TEventArgs">Type derived from <see cref="EventArgs"/>.</typeparam>
    /// <param name="eventHandler">Source <see cref="EventHandler"/> to safely invoke.</param>
    /// <param name="exceptionHandler">Custom delegate to handle encountered exceptions which includes parameter for event handler that threw the exception; when set to <c>null</c>, exception will be suppressed, see <see cref="Logger.SwallowException"/>.</param>
    /// <param name="sender">Event source.</param>
    /// <param name="args">Event arguments.</param>
    /// <param name="parallel">Call event handlers in parallel, when attached handlers are greater than one.</param>
    /// <remarks>
                ...
                case EventHandler simpleHandler:
                    ...
                    catch (Exception ex)
                    {
                        if (exceptionHandler is null)
                            Logger.SwallowException(new Exception($"SafeInvoke caught exception in {typeof(TEventHandler).FullName} event handler \"{handler.GetHandlerName()}\": {ex.Message}", ex));
                        else
